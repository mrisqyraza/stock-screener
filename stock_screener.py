...
def compute_trade_levels(df: pd.DataFrame) -> dict:
    ...
    bet (risk kecil, reward lebih besar - idealnya risk:reward >= 1:1.5).

    Logikanya:
    - Support = level support terdekat DI BAWAH harga sekarang
      (fallback: 5% di bawah harga kalau nggak ketemu level valid)
    - Stop Loss = sedikit di bawah support (buffer 2%), biar nggak
      "kena stop" cuma gara-gara noise kecil di sekitar level support
    ...
    risk = max(price - stop_loss, price * 0.01)  # minimal risk 1% biar nggak 0
