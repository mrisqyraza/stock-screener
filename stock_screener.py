...
def compute_all_indicators(df: pd.DataFrame) -> dict:
    ...
    indicators = {
        ...
        "bsjp": {
            "triggered": bool(bsjp_passed),
            "label": "Screener BSJP (Beli Sore Jual Pagi)",
            "detail": bsjp_reason,
        },
        "bpjs": {
            "triggered": bool(bpjs_passed),
            "label": "Screener BPJS (Beli Pagi Jual Sore)",
            "detail": bpjs_reason,
        },
    }
...
def fetch_news_headlines(query: str, max_articles: int = 10) -> list:
    """
    Ambil judul berita terbaru soal 'query' (biasanya nama emiten) dari
    Google News RSS - gratis, nggak butuh API key.

    Return: list of headline strings. Kalau gagal/nggak ada internet,
    return list kosong (nggak bikin program crash).
    """
    import urllib.parse
    import re as _re
    ...
    try:
        ...
        resp.raise_for_status()
        # Ambil semua isi tag <title>...</title>, skip yang pertama
        # (itu judul feed-nya sendiri, bukan judul berita)
        titles = _re.findall(r"<title>(.*?)</title>", resp.text)
        return titles[1: max_articles + 1]
    except Exception as e:
...
def compute_news_sentiment(query: str) -> dict:
    ...
    if not headlines:
        return {"triggered": False, "label": "Sentimen Berita", "detail": None, "headlines": []}
    ...
    detail = None
    if triggered:
        detail = f"Berita cenderung positif ({pos_count} sinyal positif vs {neg_count} negatif dari {len(headlines)} judul terkini)"

    return {
        "triggered": triggered,
        "label": "Sentimen Berita Positif",
        "detail": detail,
        "headlines": headlines[:5],  # buat ditampilkan kalau perlu
    }
...
def compute_fundamental_score(ticker: str) -> dict:
    ...
    return {"score": score, "reasons": reasons, "news_headlines": news.get("headlines", [])}
